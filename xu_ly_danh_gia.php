...
try {
    ...
    if ($success) {
        ...
        // --- Đặt ngay sau khi đánh giá được INSERT thành công ---
        if ($review_id = $conn->lastInsertId()) {
            $product_name = $ten_san_pham ?? 'Tên Sản Phẩm';
            $rating = $so_sao ?? 5;
            $message = "Có đánh giá mới {$rating} sao cho sản phẩm: {$product_name}.";
            $link = "admin/qldanhgia.php?review_id={$review_id}";
            $type = 'new_review';
