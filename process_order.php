...
try {
    ...
    if ($orderId) {
        $customer_name = $_SESSION['user_id']; // Giả sử bạn lưu tên khách hàng trong session
        $total_amount_formatted = number_format($total, 0, ',', '.');
        $message = "Đơn hàng mới #{$orderId} trị giá {$total_amount_formatted}₫ từ {$customer_name} vừa được đặt.";
        $link = "admin/qldonhang.php?order_id={$orderId}";
        $type = 'new_order';
    ...
    // Thêm chi tiết và giảm tồn kho (đồng bộ cho cả san_pham)
    $priceStmt = $conn->prepare("SELECT gia FROM san_pham WHERE id = ?");
    foreach ($data['items'] as $it) {
        $pid = (int)$it['id'];
        $qty = (int)$it['quantity'];
        ...
        // UPDATE trừ số lượng trong bảng san_pham (tồn kho)
        ...
        // Log thành công
        error_log("✅ Đã trừ kho: Product ID={$pid}, Số lượng={$qty}, Order ID={$orderId}");
    }
